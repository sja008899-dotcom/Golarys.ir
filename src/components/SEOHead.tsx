...
export const SEOHead: React.FC<SEOHeadProps> = ({ activeTab, selectedProduct }) => {
  useEffect(() => {
    ...
    let metaDescription = 'خرید آنلاین انواع گل تازه شاخه بریده، دسته گل رز هلندی، گیاهان آپارتمانی مقاوم، باکس هدیه لوکس و صنایع دستی سرامیکی لالجین با عکاسی قبل از ارسال، تحویل ۲ ساعته و تضمین شادابی ۷ روزه.';
    ...
    if (selectedProduct) {
      ...
      metaDescription = `${selectedProduct.description} - قیمت: ${selectedProduct.price.toLocaleString('fa-IR')} تومان. ارسال فوری اسنپ، عکس گل قبل از ارسال و ضمانت شادابی.`;
      canonicalUrl = `https://golarys.ir/product/${selectedProduct.slug}`;
      ogImage = selectedProduct.image;
    } else {
      switch (activeTab) {
        case 'marketplace':
          pageTitle = 'بازارچه آنلاین گل تازه، رز هلندی و گیاهان آپارتمانی | گل آریس (Golarys)';
          metaDescription = 'خرید مستقیم انواع دسته گل رز، ارکیده، گیاهان آپارتمانی تصفیه‌کننده هوا و باکس گل لوکس با ارسال ۲ ساعته و عکاسی قبل از ارسال.';
